...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onNavigate, user, onLogout, isDark, toggleDark, activeSession, unreadCount = 0 }) => {
  const navItems: { label: View; icon: string }[] = [
    ...
    { label: 'Assignments', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01' },
    { label: 'Assistant', icon: 'M13 10V3L4 14h7v7l9-11h-7z' },
  ];
    ...
    <aside className="fixed left-0 top-0 h-full w-20 md:w-72 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-r border-purple-100 dark:border-purple-900/50 z-50 flex flex-col transition-all shadow-2xl">
      <div className="p-8 border-b border-purple-50 dark:border-purple-900/30 flex items-center gap-4">
        ...
        <div className="hidden md:block">
          ...
          <p className="text-[9px] uppercase tracking-[0.2em] text-fuchsia-500 font-black">BVM Central</p>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-purple-50 dark:border-purple-900/30 space-y-4">
        <div className="flex gap-2">
          ...
          <button 
            onClick={onLogout}
            className="flex-1 flex items-center justify-center p-3 rounded-2xl bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 transition-all hover:bg-red-100 dark:hover:bg-red-900/50"
        ...
        <div className="flex items-center gap-3 p-3 bg-purple-50/50 dark:bg-purple-900/20 rounded-2xl group cursor-pointer border border-transparent hover:border-purple-300 transition-all" onClick={() => onNavigate('Profile')}>
          <div className="w-10 h-10 rounded-xl overflow-hidden border-2 border-white dark:border-slate-700 shadow-lg group-hover:rotate-6 transition-transform">
          ...
          <div className="hidden md:block flex-1 min-w-0">
            ...
            <p className="text-[10px] text-fuchsia-500 dark:text-fuchsia-400 font-bold tracking-tighter uppercase">{user.studentId}</p>
          </div>
        </div>
      </div>
    </aside>
